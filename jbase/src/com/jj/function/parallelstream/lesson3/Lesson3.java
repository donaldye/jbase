...
public class Lesson3 {
    ...
    static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
        ...
        //demonstrate how to do parallel processing for NxN matrix
        //use Lambdas and Stream in place of explicit loop
        intStream.forEach
            (i-> {
                    for (int j = 0; j < LIST_SIZE; j++) {distances[i][j] = Levenshtein.lev(wordList.get(i), wordList.get(j));}
                 }
            );

/*
        //do we do this to the extream?
        IntStream iiIntStream = IntStream.range(0, LIST_SIZE);
        final IntStream jIntStream;
        if (parallel) {
            jIntStream = iiIntStream.parallel();
        }
        else {
            jIntStream = iiIntStream;
        }

        intStream.forEach(
                i -> {
                    //for (int j = 0; j < LIST_SIZE; j++) {distances[i][j] = Levenshtein.lev(wordList.get(i), wordList.get(j));}}
                    iiIntStream.forEach(j -> {distances[i][j] = Levenshtein.lev(wordList.get(i), wordList.get(j));});
                });
*/
     ...
     *
     * Take the list of strings passed and process these using a sequential or parallel stream
     * as required to generate a new list. Start by simply sorting the strings then experiment
     * with adding things like mapping to lower or upper case,
     * filtering out certain words (such as those beginning with a certain letter).
     * See what impact adding distinct() to the stream has.
     * For each of these vary the size of the input list to see how this affects the performance.
     * Find the threshold below which sequential will give a faster answer than parallel.
     *
     *
     * @param wordList The subset of words whose distances to compute
     * @param parallel Whether to run in parallel
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("Computing Levenshtein ...");
